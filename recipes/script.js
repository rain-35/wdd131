...
function tagTemplate(tags) {
    let tagsHtml = tags.map(doTags).join('');

    return `<div class="recipe_tags">${tagsHtml}</div>`;

    function doTags(tag) {
        return `<div class="recipe_category">${tag}</div>`;
    }
}
...
function recipeTemplate(recipe) {
    return `
        ...
        <img class="recipe_card_img" src="${recipe.image}" alt="${recipe.name} image">
...
function displayRecipe() {
    ...
    const recipeBox = document.getElementById("recipe_box");
...
function search() {
    let recipeQuery = input.value.toLowerCase().trim();
    ...
    if (filteredRecipes.length === 0) {
        recipeBox.innerHTML = `<p>No recipes found.</p>`;
        return;
    }

    recipeBox.innerHTML = filteredRecipes.map(recipeTemplate).join('');
}
